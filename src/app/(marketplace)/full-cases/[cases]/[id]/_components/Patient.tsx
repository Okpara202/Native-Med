    ...
    <>
      ...
      <MockCasesBorder>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <MockPatientBorder bgColor="#FFF6FC" borderColor="#992E6C">
              {mockData.patientBrief
                ...
                .map((line, index) => (
                  <p
                    key={index}
                    className="text-sm sm:text-base leading-[150%] tracking-[-1%] text-black-others "
          ...
          <aside className="">
            <MockPatientBorder bgColor="white" borderColor="#D6D6D6">
              ...
              <p className="text-xs sm:text-sm leading-6">{mockData.socialHistory}</p>
            </MockPatientBorder>
          </aside>
        </div>
        ...
        <div className="space-y-4">
          <h2 className="font-semibold text-sm sm:text-base leading-[150%] tracking-[-1%] text-black-others space-y-4">
          ...
          {mockData.patientsIdeaConcernExpectation.map((item, index) => (
            <div key={index} className="flex gap-4 items-start">
              ...
              <p className="text-xs sm:text-sm text-subtle-2 leading-6">{item.desc}</p>
            </div>
          ))}
        </div>

        <MockPatientBorder bgColor="#FBE5E7" borderColor="#D70117">
          <h2 className="font-semibold text-sm sm:text-base leading-5 text-error-other">
            Important Note:
          </h2>
          <p className="text-xs sm:text-sm leading-5 text-[#C11616]">
            {mockData.importantNote}
          </p>
        </MockPatientBorder>

        <MockPatientBorder bgColor="#F4F5F7" borderColor="#5925B3">
          <h2 className="text-sm sm:text-base leading-5 font-semibold text-black-others">
            Questions for the Doctor
          </h2>
          {mockData.questionsForDoctor.map((item, index) => (
            <ul className="space-y-4" key={index}>
              <li className="text-xs sm:text-sm leading-5 list-disc list-inside text-black-others">
                {item}
              </li>
            </ul>
          ))}
