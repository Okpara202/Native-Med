...
export default function ProductsCard({ cutBy = 0 }: { cutBy?: number }) {
  ...
  return (
    <div className="grid grid-cols-3 gap-9">
      {cardToShow.map((card) => (
        <div
          className="rounded-[16px] border-[0.5px] border-[rgba(247, 244, 251, 1)] cardShadow space-y-8 overflow-y-hidden pb-4"
          key={card.id}
        >
          <aside className="relative w-full min-h-64">
          ...
          <aside className="px-6 ">
            <div className="space-y-2">
              <p className="text-xl font-semibold leading-[130%] tracking-[-1%] text-blackOthers line-clamp-2">
                {card.title}
              </p>
              <p className="line-clamp-3 text-sm leading-[150%] text-[rgba(55, 55, 55, 1)] h-28">
                {card.desc}
              </p>
            </div>
          </aside>
